 ...
 * ParadeControl.jsx
 * ─────────────────────────────────────────────────────────────
 * Combined ANO parade lifecycle dashboard.
 * Panels: Create · Permissions · Review & Close
 *
 * Bug fixes from originals:
 *  - Past-date check before setLoading (Create)
 *  - parade_instructions field in Create, ano_remarks in Review (separate fields)
 *  - Removed shadowed `today` redeclaration (Create)
 *  - Re-checks active parade at submit time (Create)
 *  - Removed duplicate `function percent()` declaration (Review)
 *  - `scopedCadets` dead variable removed (Review)
 *  - presentCadets/permissionCadets/absentCadets moved to useMemo (Review)
 *  - statusFilter now actually connected to cadet table display (Review)
 *  - All alert()/confirm() replaced with inline states + modals
 *  - "No active parade" vs "loading" states properly distinguished
 *  - Permissions locked banner shown clearly
 *  - Save/delete loading states on permission actions
 *  - Inform Seniors targets specific seniors by category+division via user_profiles table
 *  - refreshments_given toggle added to Review & Close
 */
...
const PARADE_TYPES = [
  ...
  "Parade Rehearsal", "Cultural Practice", "Event", "Awareness Program",
];

const PERMISSION_REASONS = [
  "Health issue", "Unit office work", "Went home",
  "Sports", "Camp duty", "Other",
];
...
const STYLES = `
  ...
  /* Stagger */
  .pd-card:nth-child(1){animation-delay:.04s}
  .pd-card:nth-child(2){animation-delay:.09s}
  .pd-card:nth-child(3){animation-delay:.14s}
  .pd-card:nth-child(4){animation-delay:.19s}
  ...
  .pd-input:focus,.pd-select:focus,.pd-textarea:focus {
    outline:none; border-color:var(--csi-border-focus); box-shadow:0 0 0 3px rgba(99,102,241,.11);
  }
  ...
  .pd-session-btn {
    flex:1; background:var(--csi-bg-input); border:1px solid var(--csi-border-input);
    color:var(--csi-text-sub); border-radius:0.55rem; padding:0.6rem 0.4rem;
    cursor:pointer; font-family:var(--csi-font-mono); font-size:0.7rem;
    text-align:center; transition:all .15s; display:flex; flex-direction:column; align-items:center; gap:0.18rem;
  }
  ...
  .pd-cat-card {
    flex:1; border-radius:0.7rem; border:1px solid var(--csi-border-input);
    background:var(--csi-bg-input); padding:0.85rem 0.7rem;
  ...
  .pd-cat-select {
    background:var(--csi-bg-card); border:1px solid var(--csi-border-input);
    color:var(--csi-text-primary); border-radius:0.38rem; padding:0.3rem 0.45rem;
    font-size:0.67rem; width:100%; margin-top:0.45rem; font-family:var(--csi-font-mono);
  ...
  .pd-toggle-track {
    width:40px; height:22px; border-radius:99px; border:1px solid var(--csi-border-input);
    background:var(--csi-bg-input); position:relative; cursor:pointer;
  ...
  .pd-pill {
    font-family:var(--csi-font-mono); font-size:0.63rem; padding:0.32rem 0.75rem;
    border-radius:0.4rem; border:1px solid var(--csi-border-input);
    background:var(--csi-bg-input); color:var(--csi-text-muted);
  ...
  .pd-search {
    ...
    color:var(--csi-text-primary); border-radius:0.45rem; padding:0.38rem 0.75rem;
    font-size:0.78rem; font-family:var(--csi-font-mono); width:200px; transition:border-color .15s;
  }
  ...
  .pd-table th {
    font-family:var(--csi-font-mono); font-size:0.58rem; text-transform:uppercase;
    letter-spacing:0.09em; color:var(--csi-text-muted); padding:0.55rem 0.7rem;
    text-align:left; border-bottom:1px solid var(--csi-border); background:var(--csi-bg-input);
    white-space:nowrap;
  }
  ...
  .pd-perm-chip {
    ...
    color:var(--csi-amber); border-radius:0.35rem; padding:0.2rem 0.55rem;
    font-family:var(--csi-font-mono); font-size:0.65rem; cursor:pointer;
    transition:all .12s;
  }
  .pd-perm-chip:hover { border-color:var(--csi-amber); }
  .pd-perm-add {
    display:inline-flex; align-items:center; gap:0.3rem;
    background:transparent; border:1px dashed var(--csi-border-input);
    color:var(--csi-text-muted); border-radius:0.35rem; padding:0.2rem 0.55rem;
    font-family:var(--csi-font-mono); font-size:0.63rem; cursor:pointer;
    transition:all .12s;
  }
  ...
  .pd-perm-editor {
    background:var(--csi-bg-input); border:1px solid var(--csi-border-focus);
    border-radius:0.55rem; padding:0.7rem; display:flex; flex-direction:column; gap:0.5rem;
    animation:pd-in .2s ease both;
  }
  .pd-perm-editor-row { display:flex; gap:0.4rem; align-items:center; flex-wrap:wrap; }

  /* ── Status badge ── */
  .pd-badge {
    display:inline-block; font-family:var(--csi-font-mono); font-size:0.58rem;
    font-weight:700; padding:0.15rem 0.45rem; border-radius:0.25rem; text-transform:uppercase;
  ...
  .pd-stat-box {
    flex:1; min-width:90px; background:var(--csi-bg-card); border:1px solid var(--csi-border);
    border-radius:0.7rem; padding:0.7rem 0.9rem; transition:background .25s;
  }
  ...
  /* ── Buttons ── */
  .pd-btn-primary {
    background:var(--csi-indigo); border:none; color:white;
    font-family:var(--csi-font-mono); font-size:0.82rem; font-weight:700;
    border-radius:0.6rem; padding:0.72rem 1.5rem; cursor:pointer;
    transition:background .15s, transform .1s; display:inline-flex; align-items:center; gap:0.5rem;
  }
  ...
  .pd-btn-ghost {
    background:transparent; border:1px solid var(--csi-border-input); color:var(--csi-text-sub);
    font-family:var(--csi-font-mono); font-size:0.75rem; border-radius:0.5rem;
    padding:0.45rem 1rem; cursor:pointer; transition:all .15s; display:inline-flex; align-items:center; gap:0.4rem;
  }
  .pd-btn-ghost:hover { border-color:var(--csi-indigo-light); color:var(--csi-text-primary); }

  .pd-btn-danger {
    background:var(--csi-red-bg); border:1px solid var(--csi-red-border); color:var(--csi-red);
    font-family:var(--csi-font-mono); font-size:0.82rem; font-weight:700;
    border-radius:0.6rem; padding:0.72rem 1.5rem; cursor:pointer;
    transition:all .15s; display:inline-flex; align-items:center; gap:0.5rem;
  }
  ...
  .pd-btn-icon {
    background:var(--csi-bg-input); border:1px solid var(--csi-border-input);
    color:var(--csi-text-sub); border-radius:0.4rem; padding:0.28rem 0.55rem;
    cursor:pointer; font-size:0.8rem; line-height:1; transition:all .12s;
  }
  ...
  /* ── Report card ── */
  .pd-report-card {
    border:1px solid var(--csi-border); border-radius:0.7rem; padding:0.9rem 1rem;
    cursor:pointer; transition:border-color .15s, background .15s; margin-bottom:0.6rem;
  }
  ...
  .pd-locked {
    ...
    background:var(--csi-amber-bg); border:1px solid var(--csi-amber-border);
    border-radius:0.6rem; padding:0.6rem 0.9rem; margin-bottom:1rem;
    font-family:var(--csi-font-mono); font-size:0.68rem; color:var(--csi-amber);
    animation:pd-in .22s ease both;
  }
...
function TabCreate({ onParadeCreated }) {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (active) { setBlocked(active); setInitialChecking(false); return; }
  ...
  async function handleCreate() {
    ...
    const { data: live } = await supabase.from("parades").select("id, parade_date, session")
      .in("status", ["active", "attendance_submitted"]).limit(1).maybeSingle();
    ...
    const { error: err } = await supabase.from("parades").insert({
    ...
    setSuccess({ parade_date: paradeDate, session, categories: activeCats, parade_type_map: typeMap });
    setLoading(false);
  ...
  if (blocked) return (
    <div className="pd-card pd-card--amber">
      ...
      <button className="pd-btn-ghost" style={{ marginTop: "1rem" }}
        onClick={async () => {
  ...
  /* ── Success ── */
  if (success) return (
    <div className="pd-card pd-card--green" style={{ textAlign: "center", padding: "2.25rem 1.5rem" }}>
      <div className="pd-success-icon">✓</div>
      <p style={{ ...SYNE, fontSize: "1.2rem", fontWeight: 800, color: "var(--csi-text-primary)", margin: "0 0 0.35rem" }}>Parade Created</p>
      <p style={{ ...MONO, fontSize: "0.7rem", color: "var(--csi-text-muted)", marginBottom: "1.4rem" }}>
        {success.parade_date} · {success.session?.charAt(0).toUpperCase() + success.session?.slice(1)} · Cat {success.categories?.join(", ")}
      </p>
      <div style={{ textAlign: "left", marginBottom: "1.4rem" }}>
        {Object.entries(success.parade_type_map || {}).map(([cat, type]) => (
          <div key={cat} className="pd-meta-row">
            <span className="pd-meta-key">Category {cat}</span>
            <span className="pd-meta-val" style={{ color: CAT_COLORS[cat]?.accent }}>{type}</span>
          </div>
        ))}
      </div>
      <button className="pd-btn-ghost" onClick={() => {
        setSuccess(null); setSession(""); setInstructions("");
        setSelectedCats({ A: true, B: true, C: true }); setParadeDate(today);
      }}>
        Create Another Parade
      </button>
    </div>
  );
    ...
    <div>
      ...
      <div className="pd-card">
        <p className="pd-label">
          Categories &amp; Parade Types
          <span style={{ color: "var(--csi-text-muted)", fontWeight: 400, marginLeft: "0.5rem" }}>
          ...
          {["A", "B", "C"].map(cat => {
            ...
            return (
              <div key={cat} className="pd-cat-card"
                ...
                <span className="pd-cat-badge" style={{ color: on ? col.accent : "var(--csi-text-muted)" }}>
                  {on ? "✓" : "—"}
                </span>
      ...
      <button className="pd-btn-primary pd-btn-primary--full"
        ...
        {loading ? <><div className="pd-spinner" />Creating Parade…</> : "⚡ Create Parade"}
      </button>
    </div>
  );
}
...
function TabPermissions() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      const { data: c } = await supabase.from("cadets").select("*")
        .eq("is_active", true).in("category", parade.categories)
        .order("enrollment_no");
  ...
  const getPerm = id => permissions.find(p => p.cadet_id === id);
  const permCount = filtered.filter(c => getPerm(c.id)).length;

  async function reloadPerms() {
    const { data } = await supabase.from("permissions").select("*").eq("parade_id", parade.id);
    setPermissions(data || []);
  }

  function startEdit(cadet) {
    if (locked) { setError("Permissions are locked after attendance submission."); return; }
    const ex = getPerm(cadet.id);
    setEditing({ cadetId: cadet.id, reason: ex?.reason || "", to_date: ex?.to_date || parade.parade_date, showDate: false });
    setError(null);
  }

  async function savePerm() {
    if (!editing.reason) { setError("Please select a reason."); return; }
    if (editing.to_date < parade.parade_date) { setError("Permission date cannot be before parade date."); return; }
    setSaving(true); setError(null);
    const { error: err } = await supabase.from("permissions").upsert({
      parade_id: parade.id, cadet_id: editing.cadetId,
      reason: editing.reason, to_date: editing.to_date || null,
    });
    if (err) { setError(err.message); setSaving(false); return; }
    await reloadPerms(); setEditing({}); setSaving(false);
  }
  ...
  if (loading) return (
    <div><Skeleton height={60} /><Skeleton height={300} /><Skeleton height={60} /></div>
  );
    ...
    <div>
      ...
      {locked && (
        <div className="pd-locked">
          🔒 Permissions are locked — attendance has been submitted for this parade.
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap", alignItems: "center", marginBottom: "0.75rem" }}>
        {["ALL", "A", "B", "C"].map(v => (
          <button key={v} className={`pd-pill${catFilter === v ? " pd-pill--on" : ""}`}
            onClick={() => setCatFilter(v)}>Cat {v === "ALL" ? "All" : v}</button>
        ))}
        <span style={{ color: "var(--csi-border-input)" }}>│</span>
        {["ALL", "SD", "SW"].map(v => (
          <button key={v} className={`pd-pill${divFilter === v ? " pd-pill--on" : ""}`}
            onClick={() => setDivFilter(v)}>{v === "ALL" ? "All Div" : v}</button>
        ))}
        <input className="pd-search" placeholder="Search name…" value={search}
          onChange={e => setSearch(e.target.value)} />
        <span style={{ ...MONO, fontSize: "0.62rem", color: "var(--csi-text-muted)", marginLeft: "auto" }}>
          {filtered.length} cadets · {permCount} with permission
        </span>
      </div>
            ...
            {filtered.map((cadet, idx) => {
              ...
              return (
                <tr key={cadet.id}>
                  ...
                  <td>
                    {isEditing ? (
                      <div ref={editorRef} className="pd-perm-editor">
                        <div className="pd-perm-editor-row">
                          <select className="pd-select" style={{ flex: 1, padding: "0.3rem 0.55rem", fontSize: "0.75rem" }}
                            value={editing.reason}
                            onChange={e => setEditing(ed => ({ ...ed, reason: e.target.value }))}>
                            <option value="">Select reason…</option>
                            {PERMISSION_REASONS.map(r => <option key={r}>{r}</option>)}
                          </select>
                          <button className="pd-btn-ghost" style={{ fontSize: "0.65rem", padding: "0.28rem 0.6rem", whiteSpace: "nowrap" }}
                            onClick={() => setEditing(ed => ({ ...ed, showDate: !ed.showDate }))}>
                            📅 Multi-day
                          </button>
                        </div>
                        {editing.showDate && (
                          <input type="date" className="pd-input" style={{ fontSize: "0.75rem", padding: "0.3rem 0.6rem" }}
                            value={editing.to_date} min={parade.parade_date}
                            onChange={e => setEditing(ed => ({ ...ed, to_date: e.target.value }))} />
                        )}
                        <div className="pd-perm-editor-row">
                          <button className="pd-btn-icon pd-btn-icon--save" onClick={savePerm} disabled={saving}>
                            {saving ? <div className="pd-spinner pd-spinner--sm" style={{ borderTopColor: "var(--csi-green)" }} /> : "✓ Save"}
                          </button>
                          {perm && (
                            <button className="pd-btn-icon pd-btn-icon--del" onClick={() => deletePerm(cadet.id)} disabled={saving}>
                              ✕ Remove
                            </button>
                          )}
                          <button className="pd-btn-icon" onClick={() => setEditing({})}>Cancel</button>
                        </div>
                      </div>
                    ) : perm ? (
                      <span className="pd-perm-chip" onClick={() => startEdit(cadet)}>
                        ✓ {perm.reason}{perm.to_date && perm.to_date !== parade.parade_date ? ` → ${perm.to_date}` : ""}
                      </span>
                    ) : (
                      <button className="pd-perm-add" disabled={locked} onClick={() => startEdit(cadet)}>
                        + Add
                      </button>
                    )}
                  </td>
...
function TabReview() {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase.from("parades").select("*")
        .eq("status", "attendance_submitted").maybeSingle();
      if (!data) { setNoParade(true); setLoading(false); return; }
      setParade(data);
      setRemarks(data.ano_remarks || "");
      setRefreshments(data.refreshments_given || false);
    }
    load();
  }, []);

  /* Load cadets + attendance */
  useEffect(() => {
    if (!parade) return;
    async function loadData() {
      setLoading(true);
      const { data: c } = await supabase.from("cadets")
        .select("id, enrollment_no, name, rank, category, division")
        .eq("is_active", true).in("category", parade.categories);
      const { data: a } = await supabase.from("attendance")
        .select("cadet_id, status").eq("parade_id", parade.id);
      setCadets(c || []);
      setAttendance(a || []);
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    attendance.forEach(a => {
      if (a.status === "present") {
        const c = cadets.find(x => x.id === a.cadet_id);
        if (c) s[c.rank].present++;
      }
    });
    setRankSummary(s);
  }, [cadets, attendance]);
  ...
  useEffect(() => {
    ...
    ["A", "B", "C"].forEach(cat => {
      ["SD", "SW"].forEach(div => {
        ...
        if (!attendance.some(a => scope.some(c => c.id === a.cadet_id)))
  ...
  useEffect(() => {
    ...
    async function loadReports() {
      const { data } = await supabase.from("parade_reports")
        .select("category, division, report_text, updated_at").eq("parade_id", parade.id);
      // Build map: { category: [ ...rows ] } — up to 3 rows per category (SHARED, SD, SW)
      const map = {};
      (data || []).forEach(r => {
  ...
  /* Derived cadet lists — useMemo (not mutating const arrays) */
  const { presentCadets, permCadets, absentCadets, filteredCadets } = useMemo(() => {
    const fc = cadets.filter(c =>
  ...
  async function informSeniors() {
    ...
    // For each pending slot, find the senior(s) responsible for that category+division
    // Seniors are identified via user_profiles table (role='senior', matching category+division)
    const notifRows = [];
    for (const slot of pendingSlots) {
      // Seniors are restricted by division; category assigned for system use.
      // Query matches assigned_division strictly; also checks assigned_category.
      // Seniors are restricted by division only — they handle all categories in their division
      const { data: seniorProfiles } = await supabase
        .from("profiles")
        .select("id")
        .eq("role", "senior")
        .eq("assigned_division", slot.division);

      if (seniorProfiles && seniorProfiles.length > 0) {
        seniorProfiles.forEach(s => {
          notifRows.push({
            parade_id  : parade.id,
            user_id    : s.id,
            type       : "pending",
            target_role: "senior",
            message    : `Attendance for Category ${slot.category} – ${slot.division} is pending. Submit immediately.`,
          });
        });
      } else {
        // No senior mapped for this slot — insert broadcast row without user_id
        notifRows.push({
          parade_id  : parade.id,
          type       : "pending",
          target_role: "senior",
          message    : `Attendance for Category ${slot.category} – ${slot.division} is pending (no senior assigned).`,
        });
      }
    }
  ...
  async function handleClose() {
    ...
    // Save remarks + refreshments before closing
    await supabase.from("parades").update({
      ano_remarks        : remarks,
      refreshments_given : refreshments,
    }).eq("id", parade.id);

    const user = (await supabase.auth.getUser()).data.user;
    const { error: err } = await supabase.rpc("close_parade", {
      p_actor_id  : user.id,
      p_parade_id : parade.id,
    });

    if (err) {
      if (err.message.includes("attendance_pending"))
        setError("Cannot close — attendance is still pending for some categories.");
      else if (err.message.includes("parade_not_ready"))
        setError("Parade is not ready to be closed.");
      else
  ...
  /* ── States ── */
  if (loading) return (
    <div><Skeleton height={80} /><Skeleton height={200} /><Skeleton height={120} /></div>
  );
  ...
  if (noParade) return (
    <div className="pd-card">
      <div className="pd-no-parade">
        <div className="pd-no-parade-icon">📋</div>
        <p style={{ ...SYNE, fontSize: "1rem", fontWeight: 700, color: "var(--csi-text-primary)", margin: "0 0 0.35rem" }}>
          No Parade Ready for Review
        </p>
        <p style={{ ...MONO, fontSize: "0.72rem", color: "var(--csi-text-muted)" }}>
          Parade review is available once seniors have submitted attendance.
        </p>
      </div>
    </div>
  );
  ...
  if (closed) return (
    <div className="pd-card pd-card--green" style={{ textAlign: "center", padding: "2.5rem 1.5rem" }}>
      ...
      <p style={{ ...MONO, fontSize: "0.7rem", color: "var(--csi-text-muted)" }}>
        All records have been locked permanently.
      </p>
    </div>
  );

  /* Cadet list filtered by statusFilter for display */
  ...
  return (
    <div>
        ...
        <div className="pd-card pd-card--amber" style={{ marginBottom: "0.9rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap", marginBottom: "0.9rem" }}>
            {pendingSlots.map((s, i) => (
              <span key={i} className="pd-badge pd-badge--absent">
                Cat {s.category} – {s.division}
              </span>
            ))}
          </div>
          {notified
          ...
          <table className="pd-rank-table">
            ...
            <tbody>
              ...
              <tr>
                <td>Present</td>
                {RANK_ORDER.map(r => (
                  <td key={r} style={{ color: rankSummary[r]?.present ? "var(--csi-green)" : "var(--csi-text-muted)" }}>
                    {rankSummary[r]?.present || 0}
                  </td>
                ))}
              </tr>
            </tbody>
      ...
      <div className="pd-card">
        ...
        {["A", "B", "C"].map(cat => {
          const rows = reports[cat] || [];          // array of submitted reports
          ...
          return (
            <div key={cat}
              className={`pd-report-card${!hasAny ? " pd-report-card--missing" : ""}`}
              onClick={() => hasAny && setOpenReport(cat)}>
              ...
              {hasAny && (
                <p style={{ fontSize: "0.78rem", color: "var(--csi-text-sub)", margin: "0.2rem 0 0", overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                  {preview}
                </p>
              )}
              {hasAny && <p style={{ ...MONO, fontSize: "0.58rem", color: "var(--csi-text-muted)", margin: "0.3rem 0 0" }}>
      ...
      {/* Report modal — shows all submitted reports for a category */}
      {openReport && (() => {
            ...
            <div className="pd-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 700, maxHeight: "85vh", overflowY: "auto" }}>
              ...
              {sharedRow && (
                <div style={{ marginBottom: divRows.length ? "1.2rem" : 0 }}>
                  <p style={{ ...MONO, fontSize:"0.62rem", fontWeight:700, color:"var(--csi-indigo-light)", marginBottom:"0.5rem", textTransform:"uppercase" }}>
                    Shared Report (All Divisions)
                  </p>
                  <p style={{ whiteSpace:"pre-wrap", fontSize:"0.83rem", color:"var(--csi-text-sub)", lineHeight:1.65, marginBottom:"0.4rem" }}>
                    {sharedRow.report_text}
                  </p>
                  <p style={{ ...MONO, fontSize:"0.58rem", color:"var(--csi-text-muted)" }}>
                    Updated: {new Date(sharedRow.updated_at).toLocaleString()}
                  </p>
                </div>
              )}
              ...
              {divRows.map((row, i) => (
                <div key={row.division} style={{ marginTop: i > 0 || sharedRow ? "1.2rem" : 0, paddingTop: i > 0 || sharedRow ? "1.2rem" : 0, borderTop: i > 0 || sharedRow ? "1px solid var(--csi-border)" : "none" }}>
                  <p style={{ ...MONO, fontSize:"0.62rem", fontWeight:700, color:"var(--csi-green)", marginBottom:"0.5rem", textTransform:"uppercase" }}>
      ...
      <div className="pd-card">
        <div className="pd-filter-row">
          ...
          {[
            ...
            <button key={v} className={`pd-pill${statusFilter === v ? " pd-pill--on" : ""}`}
        ...
        <div className="pd-stat-grid">
          {[
            ...
            <div key={k} className="pd-stat-box">
              <p className="pd-stat-key">{k}</p>
              <p className="pd-stat-val" style={{ color: c }}>{v}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Cadet tables */}
      {displayPresent && presentCadets.length > 0 && (
        <MiniCadetTable title="Present" cadets={presentCadets} badge="present" />
      )}
      {displayPerm && permCadets.length > 0 && (
        <MiniCadetTable title="Absent — With Permission" cadets={permCadets} badge="perm" />
      )}
      {displayAbsent && absentCadets.length > 0 && (
        <MiniCadetTable title="Absent — No Permission" cadets={absentCadets} badge="absent" />
      )}
      ...
      <div className="pd-card">
        ...
        <textarea className="pd-textarea"
          placeholder="Final remarks for this parade…"
          value={remarks}
          onChange={e => setRemarks(e.target.value)}
          disabled={parade.status === "completed"}
        />
        <p style={{ ...MONO, fontSize: "0.62rem", color: "var(--csi-text-muted)", marginTop: "0.4rem" }}>
      ...
      <div className="pd-card">
        ...
        <div className="pd-toggle-row">
          <div>
            <div style={{ ...MONO, fontSize: "0.72rem", color: "var(--csi-text-primary)", fontWeight: 600, marginBottom: "0.15rem" }}>
              Refreshments Provided
            </div>
            <div style={{ ...MONO, fontSize: "0.6rem", color: "var(--csi-text-muted)" }}>
              Were refreshments given to cadets after this parade?
            </div>
          </div>
          <div className={`pd-toggle-track${refreshments ? " pd-toggle-track--on" : ""}`}
            onClick={() => setRefreshments(r => !r)}>
      ...
      {pendingSlots.length > 0 && (
        <p style={{ ...MONO, fontSize: "0.65rem", color: "var(--csi-text-muted)", textAlign: "right", marginTop: "0.4rem" }}>
          Cannot close — attendance still pending
        </p>
      )}
